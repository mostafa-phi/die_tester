...
| MISUMI **LX2005CG-B1-A2040-300** (X), **-200** (Y), **-100** (Z) single-axis actuators | `misumi_LX2005CG-B1-A2040-{300,200,100}.step` | **placed** (`lx_file_members()`: body with the motor-bracket casting + cover strip, table plate, adapter plate + screws; the motor is an envelope on the plate). The files confirmed the catalog reading: 40 wide × 26.5 tall body, 52 × 57 table plate to 27 above the rail bottom, screw axis at 13, bracket casting 56 beyond the rail, 40 × 40 × 13 adapter plate hanging 7 mm below the rail. Effective stroke = L − 63.5. The uploaded files carry the A2040 (□40 servo) adapter; a 42 sq stepper needs the T2042 plate, same outline. |
...
4. The motor on each LX20 (the files carry the A2040 servo adapter; the model's motor envelope is 42 sq × 90 long, which covers a 40 sq 50 W servo or a 42 sq brake stepper on the T2042 plate) and its cable exit.
