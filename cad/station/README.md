...
static moments on the X and Z table plates against the LX20's 27 / 27 / 93 N·m pitch / yaw / roll ratings, the exchange time at
200 mm/s). The moments are r × F from each mass's centre: the 6061 parts (arm, tower bracket, live-view boom) are weighed from their CAD
volume at 2.70 g/cm³, the bought parts keep their catalog or estimated masses (`S["bought_mass"]`: Z actuator 0.45, brake motor 0.6,
gripper module 0.1, cameras 0.1 kg) at the centre of their envelope, and the X move's 0.8 m/s² ramp adds its d'Alembert moments in
the worst sign. The reference point is the centre of each table plate's face; the catalog's own moment reference is still to be
confirmed. An earlier version multiplied the whole tower weight by its height above the table, which is not a gravity moment, and
booked the arm at 0.25 kg against 0.67 kg in the CAD.
...
| `arm_6061[_h]` | one piece: 33-deep adapter block on the Z table plate, 25 sq bar along +Y, 8 mm end plate over the gripper bracket | block: 4 × Ø4.5 counterbored Ø8 from the outside + 2 × Ø3 dowels; end plate: 4 × M4 tap-drill + 2 × Ø3 dowels on `gripper.IFACE`, 4 × M2 + Ø17 for the tray camera, 2 × M3 tap-drill in the −Y edge for the live-view boom |
...
| `live_cam_boom_6061` | boom for the live-view camera off the end plate's −Y edge: foot, leg, shelf and a 55°-tilted camera pad, one piece (printed for the prototype) | foot: 2 × Ø3.4 along Y (M3 into the end plate's edge, tap-drill in `arm_6061`); pad: Ø17 for the lens ring + 4 × M2 tap-drill on 22.4 sq |
...
## Cameras on the arm (`SN` in `model.py`, the dart housing in `common.SENSORS`)
...
|---|---|---|---|
| Tray camera | **Basler dart daA1440-220um S-mount** (IMX273 global shutter 1440 × 1080, 3.45 µm, USB3, pypylon, 15 g) + 16 mm M12 lens on a 4 mm spacer ring (a board lens is sold focused near infinity with a 100–200 mm minimum object distance; 4.3 mm of extension focuses it at 76 mm, magnification 0.27, depth of field ≈ 1.5 mm at f/4) | standing on top of the end plate, front face on the plate, lens ring down through a Ø17 hole, 4 × M2 from above; optical axis 76 mm behind the die origin on the pick line (X −76, Y 3), lens front 56 above the jaws' die-bottom plane | at the traverse height a tray die top is 75.6 mm away: field 18.5 × 13.9 mm, 13 µm/px, one pocket per frame. Pocket occupied / empty, die orientation from a fiducial, chipped corners, the die's Y offset in the pocket to ±0.05 mm so the pick can be corrected, and the die's height from its apparent size (a 0.1 mm Z change scales the 770-pixel die image by 1.0 px; ±0.03 mm with sub-pixel edges after a one-time lens calibration). Column 0 is imaged with the jaws at die X −19 (inside the exchange envelope), the far column at −131 |
| Live-view camera | a second **dart daA1440-220um** + 8 mm M12 lens on a 1.5 mm spacer (focused at 46 mm, magnification 0.21; the short focal length is chosen for depth of field, ≈ 6 mm at f/5.6 with a 3-pixel blur, since the die is seen tilted and the jaws move 2 mm toward it) | on a boom (`live_cam_boom_6061`) off the end plate's −Y edge: the camera hangs under a shelf at −Y of the jaws, front face tilted, optical axis in the die's X = 5 plane at **35° above the horizontal**, aimed at the die centre from 63 mm (lens front at X 5, Y −35, Z 27 in the gripper frame) | a side view of both jaws and the die between them, 24 × 18 mm at 16 µm/px, the same picture at every pocket and at the nest: the noses entering the slots, the die lifting off the ledges or seating on the pads, the release. Monitoring and fault diagnosis, not metrology (the microscope and the tray camera measure). At the nest the sight line to the die's near bottom edge passes 0.2 mm over the input chuck tip, so the chuck hides only what is below the die in the middle of the frame |

The tray camera body (X −91…−61, Y −12…18, Z 78…92, lens down to Z 56) stands on the plate between the arm bar and the actuator; nothing is added near the jaws. At the nest it is clear of the holders and the objective; at the far column it is beside the tower (no Y overlap) and above the deck (`checks.txt`, "tray sensors" lines). The live-view boom is one 6061 part: a foot 6 × 16 mm bolted to the end plate's −Y edge with 2 × M3 along Y (foot X −28…−12, kept 3 mm from the arm bar and 2.5 mm above the gripper bracket's top plate), a 4 mm leg down to a shelf at Z 63…67 (Y −52…−24, its +Y edge 7 mm from the Ø40 microscope tube), and a 36 × 52 × 4 pad tilted 55° from the horizontal whose upper end runs into the shelf; the dart's front face is bolted to the back of the pad with 4 × M2, ring through a Ø17 hole. Camera + lens + boom ≈ 0.09 kg; the boom's members are checked one by one (`checks.txt`, "live_cam_boom" lines: nearest static object the input rotator, 8 mm below the lens; the camera moves with the gripper and stays in the exchange envelope's Y band). Both USB3 micro-B cables run along the arm bar to the tower. Lighting for the live view: two 3 mm white LEDs on the pad beside the lens, or the microscope ring light at the nest and the bench light at the tray.
...
| `live_cam_dart`, `live_cam_lens`, `live_cam_usb_plug`, `live_cam_boom_6061` | live-view camera and its boom (`live_cam_fov`, the field-of-view aid, only in `gripper_with_sensors.step`, which also carries `fiber_chuck_in` / `fiber_rotator_in` for the view) |
