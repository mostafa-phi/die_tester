...
bench with the parts. Design record: `../../../README.md` (section R04).
...
| fixture | gauge shims / feeler stock for the void gaps (see below), or the jig plate | |
| screws | M2 × 6 A2 on the 14 screwed bars, torque 0.3 N·m, Loctite 243 | the 2 inner guide bars have no screws: spring clip during cure |
...
1. Bolt the frame to the base plate (4 × M4, finger tight).
2. Put gauge shims of the nominal gap in the voids so the moving pieces cannot
   drift: **8.5 mm** between each platform arm and its stage (the coupler free
   length), and **8.5 mm** between each stage top and the frame (the guide free
   length); check the stage is square to the frame with a small square.
3. The platform's front face and the stages' front faces should lie flush
   with the frame's front face (all pieces are the same 8 mm plate): lay a flat
   plate across the front during cure.
...
3. **Bond**, one tab at a time: a toothpick's worth of mixed epoxy on the ledge,
   leaf on, bar on, screws to 0.3 N·m (or spring clip on the two bonded-only
   bars). Wipe squeeze-out at the bar edge with an IPA-damp swab. Target bond
   line 0.05–0.10 mm; the bar torque gives that by itself.
...
6. Remove the gauge shims and the front plate. Push the platform gently by
