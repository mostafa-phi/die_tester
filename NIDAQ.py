...
class USB6363:
    ...
    def __init__(self):
        ...
        self.ai_task = []
        self.di_task = []
        self.ao_task = [] # added for analog output
    ...
    def add_analogOutput(self, channel):
        try:
            self.ao_task = nidaqmx.Task()
            self.ao_task.ai_channels.add_ai_voltage_chan(self.device_name + "/" + channel)
        except Exception as e:
            print(f"Error Adding an Analog Output Channel: {e}")
            raise   
    ...
    def write(self,x):
        ...
        try:
            if self.ao_task:
                self.ao_task.write(x, auto_start = True)
        except Exception as e:
            print(f"Error Reading the tasks: {e}")
            raise  
