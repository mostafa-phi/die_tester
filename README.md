...
- [Parts list](docs/parts_list.md) — one page sorting every part into purchase, machine (with material and process) and print (PPA-CF), with the CAD file for each.
- [Print list](docs/print_list.md) — every custom part with its STL, what is printed in PPA-CF for the prototype and what is machined, print orientation, holes to ream or tap, bolts, and the order to print in.
...
- [Interactive 3-D clearance model](docs/die_handling_3d.html) — three.js scenes of the test-station exchange (nest on its die stage, NanoMax fiber stages, microscope, the three MISUMI LX20 actuators with tower and arm, wafer tray) and of the place into a tray pocket, with the clearances from the CAD checks.
