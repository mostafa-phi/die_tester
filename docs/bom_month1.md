...
|---|---|---|---|---|---|---|---|
| A1 | Transfer axes (prototype **and** production kinematics) | **Bench‑level 3‑axis Cartesian from three MISUMI LX20 single‑axis actuators**, sized to what the handling needs (±0.15 mm at the nest, ±0.3 mm at the tray, ±0.05 mm in Z; the die is placed by hard references, every position approached from the same side): **LX2005CG‑B1‑T2042‑300** (X, 236.5 mm stroke, 209 used), **LX2005CG‑B1‑T2042‑200** (Y, 136.5 mm, 97.5 used), **LX2005CG‑B1‑T2042‑100** (Z, 36.5 mm, 20 used). High grade (±5 µm repeatability), one long block, **lead 5** (690 mm/s screw‑rated, ~200 mm/s with the motor below), **cover included and low‑particulate grease** (the Z axis hangs over the nest), motor adapter **T2042** for a 42 mm square stepper (the CAD files were configured with **A2040**, the □40 servo adapter: same casting and outline, only the plate's hole pattern differs, so pick the adapter with the motor). Motors, stepper route: Oriental Motor αSTEP **AZM46AK** (X, Y) and **AZM46MK with brake** (Z: a lead‑5 ball screw back‑drives under the tower), three **AZD‑KD** drivers or one multi‑axis AZD‑KX. Servo route on A2040: 50 W □40 servos (Mitsubishi HF‑KP053 / Yaskawa SGM7J‑A5, brake type on Z) with their amplifiers, ~2× the stepper price for 3000 rpm (250 mm/s at lead 5) and a real encoder. Home/limit by the LX20 sensor‑rail proximity set (DG20X flag kit + 3 × GXL‑8F). The three MISUMI STEP files are placed in `cad/station` (`cad/vendor`, git‑ignored). **Fallbacks:** Velmex BiSlide MN10‑0100/0050 + VXC (~$4–5 k, 5 µm, 2–3 wk); FUYU FSL40 / igus drylin modules on a G‑code board (~$1 k, 20–50 µm, no moment ratings). | MISUMI USA (configurator, 3–5 days); Oriental Motor USA | actuators ~$550 + $500 + $460 (list, high grade; +$40 each for the G grease is included in the part number); motors + drivers **quote** (~$1.5–2 k); ≈ **$3–3.5 k** for the three axes | 1 set | MISUMI 3–5 days (11 for F/N types); Oriental Motor in stock | The tool must enter a ~13 mm corridor under the objective horizontally along die X and never cross the fiber line. The X actuator lies beside the input NanoMax at Y −140 on one riser bar, its 52 mm band outside the tray's Y sweep, so nothing passes under it; the Z actuator stands on an angle bracket on the X table; the arm is one 25 mm bar along +Y. No θ axis. Same kinematics scale to the production cell (`cad/station/README.md`). |
