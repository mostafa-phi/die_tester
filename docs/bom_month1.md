...
| A4 | Test nest (self-registering, `docs/cad/nest_module.py`) | **Custom, three parts**: (a) 17‑4 PH H900 (or 6061 hard‑anodized) **rail insert** — two rails 0.9 × 1.5 × 10 mm at 1.0 mm inboard of the facet planes, rail tops lapped flat ≤ 5 µm, 4 × Ø0.6 mm vacuum ports per rail into a plenum and M5 port; (b) **Semitron ESd 480 cage frame** with four corner blocks: two +X stop pads (Y 0.6–1.2 / 4.8–5.4, face at X 10.000, contact band Z 0.05–0.40) that fix X and yaw when the gripper pushes the die home with 0.25 N, a −X guard at 0.4 mm and Y guards 0.6 mm outside the facets at the die corners only; (c) 6061 riser on the B3 kinematic base. Replaces the X‑Y‑θ correction stage | Protolabs / Xometry CNC (lapping in‑house on a granite plate with 3 µm diamond film, or a local lapping shop) | quote (~$400–800) | 1 (+1 spare rail insert if two‑piece) | 1 wk | The one precision part. Two‑piece (base + rail insert) is recommended so rails can be re‑lapped or re‑made without the base. |
