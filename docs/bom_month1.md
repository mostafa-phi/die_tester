...
| B14 | Axis mounting | Thorlabs / Zaber adapter plates and a 90° bracket for the Z stage; 86 mm aluminum riser under the X axis (two pedestals with a gap for the Y stage and the tray deck) so its body sits above the NanoMax envelope and the gripper arm meets nest and tray heights (`cad/station`) | Thorlabs / McMaster + in‑house tapping | ~$250 | 1 | X axis runs along the exchange direction beside the input NanoMax (centre-line Y −100), outside both fiber corridors. |
